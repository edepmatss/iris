		...
		icon: (
			...
			>
				<path
					strokeLinecap="round"
					strokeLinejoin="round"
					d="M4 6v13a1 1 0 001 1h14M8 14l3-3 4 4 5-5"
				/>
			</svg>
		),
	},
	...
	return (
		...
		>
			<div className="flex flex-col h-full overflow-hidden">
				<div
					className={`pt-16 pb-10 flex z-10 shrink-0 transition-all duration-300 ${isOpen ? "px-12 justify-start" : "px-0 justify-center"}`}
				>
				...
				<nav className="flex flex-col w-full z-10">
					{currentNavItems.map((item, index) => {
						...
						return (
							...
							>
								<div
									className={`shrink-0 transition-transform duration-300 ${isActive ? "text-white" : "text-[#9ca3af]"}`}
								>
									{item.icon}
								</div>
								...
								{!isOpen && !isAdmin && (
									<span
										className={`text-[20px] ${isActive ? "font-bold text-white" : "font-normal text-[#9ca3af]"}`}
									>
										{index + 1}
									</span>
								)}
				...
				{isAdmin && (
					<div
						className={`mt-auto mb-10 z-10 w-full flex flex-col ${isOpen ? "px-8" : "px-0 items-center"}`}
					>
						<Link
							to="/"
